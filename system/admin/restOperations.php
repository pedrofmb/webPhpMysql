...
switch ($opcion)
{
    ...
    {
        ListRegistrations($_POST["eventPage"]);
        break;
    }
    case "exportToExcel":
    {
...
function ListRegistrations($_eventPage)
{
    global $link;
    $array = array();
    $sql = "SELECT A.*, B.nameType FROM registration A 
            INNER JOIN eventtype B ON A.idEvent = B.id 
            INNER JOIN eventpage C ON B.idPage = C.id
            WHERE C.id = ". $_eventPage ." ORDER BY registerDateTime DESC";
    $result = mysql_query($sql, $link);
